const view = {
    ...
    if (validity.typeMismatch) {
      ...
      for (const invalidKey in customMessages) {
        if (validity[invalidKey]) {
          const customMessage =
            invalidKey === "tooShort"
              ? customMessages[invalidKey][type]
              : customMessages[invalidKey];
  ...
  checkInputValidity: function (input, customMessages) {
    ...
    if (input.name == "registrationConfirmPassword") {
      ...
      const message = view.getCustomErrorMessage(
        input.type,
        input.validity,
        customMessages
      );
      const errorMessageLabel = input.parentElement.querySelector(
        ".form-error-message"
      );
  ...
  checkTabValidity: function (formTabs) {
    const currentTabInputs = document.querySelectorAll(
      "form.active .form-tab.active > div > input"
    );
    let validInputs = 0;
  ...
  setUpEventListeners: function () {
    ...
    const registrationPassword =
      view.registrationForm.elements["registrationPassword"];
    const registrationFormTabs =
      view.registrationForm.getElementsByClassName("form-tab");
    const registrationSubmitSection = document.getElementById(
